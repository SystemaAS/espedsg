  ...
  function getItemData(record) {
		var FIELD_SEPARATOR = "_";
	  	...
	  	jq.ajax({
	  	  type: 'GET',
	  	  url: 'getFrisokveiDetailLine_Flyimport.do',
			...
			buttons: {
		        Fortsett: function() {
	      		jq( this ).dialog( "close" );
		            //do delete
		            jq.blockUI({ message: BLOCKUI_OVERLAY_MESSAGE_DEFAULT});
		            window.location = "tror_mainorderfly_freightbill_list_edit.do?action=doDelete" + "&dfavd=" + avd + "&dfopd=" + opd + "&dffbnr=" + id + "&sign=" + sign ;
		        },
		        Avbryt: function() {
		            jq( this ).dialog( "close" );
		        }
