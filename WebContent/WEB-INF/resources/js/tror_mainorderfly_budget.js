  ...
  jq(function() {
	  jq('#updCancelButton').click(function() {
  		window.open('tror_mainorderfly_budget.do?avd='+ jq('#avd').val() + '&opd=' + jq('#opd').val() + "&tur=" + jq('#hepro').val(), 'budgetWin','top=120px,left=100px,height=800px,width=1600px,scrollbars=no,status=no,location=no');
	  });
  });
  ...
  jq(function() {
	...
	jq("#bulnr").blur(function() { 
			jq.ajax({
		  	  type: 'GET',
		  	  url: 'validateSupplier_Flyimport.do',
  ...
  function getBudgetItemData(record) {
		var FIELD_SEPARATOR = "_";
	  	...
	  	jq.ajax({
	  	  type: 'GET',
	  	  url: 'getBudgetDetailLine_Flyimport.do',
      ...
      buttons: {
	        Fortsett: function() {
      		jq( this ).dialog( "close" );
	            //do delete
	            jq.blockUI({ message: BLOCKUI_OVERLAY_MESSAGE_DEFAULT});
	            window.location = "tror_mainorderfly_budget_edit.do?action=doDelete" + "&bubnr=" + bubnr + "&avd=" + avd + "&opd=" + opd + "&tur=" + tur;
	        },
	        Avbryt: function() {
	            jq( this ).dialog( "close" );
	        }
