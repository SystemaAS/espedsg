	...
	*/
	...
	function getInvoiceItemData(record) {
		var FIELD_SEPARATOR = "_";
	  	...
	  	jq.ajax({
	  	  type: 'GET',
	  	  url: 'getOrderInvoiceDetailLine_Flyimport.do', //we are borrowing services from TranspDisp in the Controller until Tror module is able to migrate this ...
	  ...
	  jq('<div></div>').dialog({
        ...
        buttons: {
	        Fortsett: function() {
	            ...
	            //do delete
	            jq.blockUI({ message: BLOCKUI_OVERLAY_MESSAGE_DEFAULT});
	            window.location = "tror_mainorderflyimport_invoice_edit.do?action=doDelete" + "&heavd=" + heavd + "&heopd=" + heopd + "&fali=" + fali;
	        },
	        Avbryt: function() {
	            jq( this ).dialog( "close" );
	        }
