...
<!-- =====================end header ==========================-->
	...
	<SCRIPT type="text/javascript" src="resources/js/tvinnsadmaintenancefelles.js?ver=${user.versionEspedsg}"></SCRIPT>	
						...
						<table id="mainList" class="display compact cell-border" >
				            ...
				            <c:forEach var="record" items="${model.list}" varStatus="counter">   
				               ...
				               <td width="2%" class="tableCell" style="border-style: solid;border-width: 0px 1px 1px 0px;border-color:#FAEBD7;" align="center">
				               	<c:choose>
				               		<c:when test="${record.status == 'G' && not empty record.pgm}">
					               		<a id="alinkRecordId_${counter.count}" onClick="setBlockUI(this);" href="tvinnsadmaintenancefelles_${record.pgm}.do?id=${record.dbTable}">
				               ...
				               <td class="tableCell" style="border-style: solid;border-width: 0px 1px 1px 0px;border-color:#FAEBD7;" width="20%" >
				               		<c:choose>
					               		<c:when test="${record.status == 'G' && not empty record.pgm}">
					               			<a id="alinkRecordDesc_${counter.count}" onClick="setBlockUI(this);" href="tvinnsadmaintenancefelles_${record.pgm}.do?id=${record.dbTable}">
