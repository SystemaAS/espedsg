...
<!-- =====================end header ==========================-->
	...
	<SCRIPT type="text/javascript" src="resources/js/tvinnsadimport_edit_notisblock.js?ver=${user.versionEspedsg}"></SCRIPT>
			...
			<c:choose>
				<c:when test="${not empty model.orig && 'topic'==model.orig}">
					<td width="12%" valign="bottom" class="tabDisabled" align="center" nowrap>
						<a id="alinkTopicList" style="display:block;" 
							<c:choose>
								<c:when test="${empty model.sign}">href="tvinnsadimport.do?action=doFind&sg=${user.tvinnSadSign}"</c:when>
								<c:otherwise>href="tvinnsadimport.do?action=doFind&sg=${model.sign}"</c:otherwise>
							</c:choose> > 
					...
					<td width="12%" valign="bottom" class="tabDisabled" align="center" nowrap>
						<a id="alinkHeader" style="display:block;" href="tvinnsadimport_edit.do?action=doFetch&avd=${model.avd}&opd=${model.opd}
								&sysg=${model.sign}&tuid=${refnr}&syst=${model.status}&sydt=${model.datum}&o2_sist=${ model.o2_sist}&o2_sidt=${ model.o2_sidt}">
					...
					<td width="12%" valign="bottom" class="tabDisabled" align="center" nowrap>
						<a id="alinkInvoices" style="display:block;" href="tvinnsadimport_edit_finansopplysninger.do?action=doFetch&avd=${ model.avd}&sign=${ model.sign}
													&opd=${ model.opd}&status=${ model.status}&fabl=&o2_sist=${ model.o2_sist}&o2_sidt=${ model.o2_sidt}">
					...
					<td width="12%" valign="bottom" class="tab" align="center" nowrap>
						<font class="tabLink">&nbsp;<spring:message code="systema.tvinn.sad.import.notisblock.createnew.tab"/></font>
					...
					<td width="1px" class="tabFantomSpace" align="center" nowrap><font class="tabDisabledLink">&nbsp;</font></td>
					<td width="12%" valign="bottom" class="tabDisabled" align="center" nowrap>
						<a id="alinkLogging" style="display:block;" href="tvinnsadimport_logging.do?avd=${ model.avd}&sign=${ model.sign}&opd=${model.opd}&status=${model.status}&o2_sist=${ model.o2_sist}&o2_sidt=${ model.o2_sidt}">
							<font class="tabDisabledLink">
				...
				<c:otherwise>
					<td width="15%" valign="bottom" class="tabDisabled" align="center" nowrap>
						<a style="display:block;" href="tvinnsadimport.do?action=doFind&sg=${model.sign}">
							<font class="tabDisabledLink">&nbsp;<spring:message code="systema.tvinn.sad.import.list.tab"/></font>
							<img valign="bottom" src="resources/images/list.gif" border="0" alt="general list">
						</a>
					</td>
 	...
 	<table width="100%" class="tabThinBorderWhite" border="0" cellspacing="0" cellpadding="0">
		...
		<table border="0" width="95%" align="center">
						...
						<tr >
							...
							<form name="createNewItemLine" id="createNewItemLine" method="post" action="editNotisblock.do">
								<%--Required key parameters --%>
							 	...
							 	<input type="hidden" name="subsys" id="subsys" value='sadi'/>
								...
								<table width="80%" cellspacing="0" border="0" cellpadding="0">
									<tr>
										<td class="text12Bold">
											<input tabindex=-1 class="inputFormSubmitStd" type="submit" name="submit" onclick="javascript: form.action='editNotisblock.do';" value="<spring:message code="systema.tvinn.sad.import.item.line.init.createnew.submit"/>">
										</td>
									</tr>
									<tr>
					               ...
					               <form name="formItemList" id="formItemList" method="POST" >
				 					  ...
				 					  <c:forEach items="${model.list}" var="record" varStatus="counter">    
									       ...
									       <td class="tableCell" align="center" nowrap>
							               	<a onclick="javascript:return confirm('Er du sikker på at du vil slette denne?')" tabindex=-1 href="editNotisblock.do?action=doDelete&subsys=sadi&orig=${model.orig}&sign=${model.sign}&frtli=${record.frtli}&frtdt=${record.frtdt}&opd=${record.frtopd}&avd=${record.frtavd}">
			...
			<%-- ------------------------------------------------- --%>
           	...
           	<tr>
	 			<td >
	 				<form name="sadImportEditTopicNotisblockItemForm" id="sadImportEditTopicNotisblockItemForm" method="post">
				 	<%--Required key parameters --%>
