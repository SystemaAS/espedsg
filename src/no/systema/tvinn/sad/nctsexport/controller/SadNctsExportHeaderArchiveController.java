...
/**
 * 
 * SAD-NCTS Export Topic Archive Controller 
 * 
...
@Controller
@Scope("session")
public class SadNctsExportHeaderArchiveController {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(SadNctsExportHeaderArchiveController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadnctsexport_archive.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("tvinnsadnctsexport_archive");
		logger.info("Method: doShowList [RequestMapping-->tvinnsadnctsexport_archive.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = SkatNctsExportUrlDataStore.NCTS_EXPORT_BASE_ARCHIVE_FOR_SPECIFIC_TOPIC_URL;
			//url params
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadnctsexport_renderArchive.do", method={ RequestMethod.GET })
	public ModelAndView doSadNctsExportRenderArchive(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doSadNctsExportRenderArchive...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
	 ...
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonSkatNctsExportSpecificTopicArchiveRecord record){
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonSkatNctsExportSpecificTopicArchiveContainer container){
	 ...
	 * @param jsonNctsExportSpecificTopicRecord
	 */
	private void setDomainObjectsInView(Map model, JsonSkatNctsExportSpecificTopicArchiveRecord record){
		//SET HEADER RECORDS  (from RPG)
		model.put(SkatConstants.DOMAIN_RECORD, record);
	}
	/**
