...
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002Kodts4rValidator;
...
@Scope("session")
public class MaintSadImportSad002Kodts4Controller {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSad002Kodts4Controller.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts4r.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts4r");
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodts4Record> list = this.fetchList(appUser.getUser(), id);
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts4r_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodts4Record recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts4r");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002Kodts4rValidator validator = new MaintSadImportSad002Kodts4rValidator();
			...
			if(bindingResult.hasErrors()){
				...
				if (TvinnSadMaintenanceConstants.ACTION_UPDATE.equals(action) ){
					...
					recordToValidate.setKs4trm(null);
				}
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodts4Record> fetchList(String applicationUser, String ks4trm){
		...
		if(ks4trm!=null && !"".equals(ks4trm)){
			urlRequestParams.append("&ks4trm=" + ks4trm);
		}
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodts4Record record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTS4R_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodts4Container container = this.maintSadImportKodts4Service.doUpdate(jsonPayload);
	        if(container!=null){
