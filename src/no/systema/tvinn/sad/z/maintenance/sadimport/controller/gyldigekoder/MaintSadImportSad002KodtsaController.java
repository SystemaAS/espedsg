...
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002KodtsarValidator;
...
public class MaintSadImportSad002KodtsaController {
	...
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodtsaRecord> list = this.fetchList(appUser.getUser(), id);
	...
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodtsaRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodtsar");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002KodtsarValidator validator = new MaintSadImportSad002KodtsarValidator();
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodtsaRecord> fetchList(String applicationUser, String ks7tln){
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodtsaRecord record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTSAR_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodtsaContainer container = this.maintSadImportKodtsaService.doUpdate(jsonPayload);
	        if(container!=null){
