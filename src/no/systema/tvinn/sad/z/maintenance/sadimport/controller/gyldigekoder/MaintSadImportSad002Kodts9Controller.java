...
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002Kodts9rValidator;
...
@Scope("session")
public class MaintSadImportSad002Kodts9Controller {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSad002Kodts9Controller.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts9r.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts9r");
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodts9Record> list = this.fetchList(appUser.getUser(), id);
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts9r_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodts9Record recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts9r");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002Kodts9rValidator validator = new MaintSadImportSad002Kodts9rValidator();
			...
			//FETCH table
			//------------
	    	List<JsonMaintSadImportKodts9Record> list = this.fetchList(appUser.getUser(), recordToValidate.getKs9typ());
	    	//set domain objets
	    	model.put("dbTable", dbTable);
			model.put(TvinnSadMaintenanceConstants.DOMAIN_LIST, list);
			successView.addObject(TvinnSadMaintenanceConstants.DOMAIN_MODEL , model);
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodts9Record> fetchList(String applicationUser, String ks9typ){
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodts9Container container = this.maintSadImportKodts9Service.getList(jsonPayload);
	        if(container!=null){
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodts9Record record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTS9R_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodts9Container container = this.maintSadImportKodts9Service.doUpdate(jsonPayload);
	        if(container!=null){
