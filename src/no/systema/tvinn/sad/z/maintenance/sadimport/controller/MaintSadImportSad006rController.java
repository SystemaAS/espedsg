...
import no.systema.tvinn.sad.z.maintenance.sadimport.service.MaintSadImportSadhHeadfService;

import no.systema.tvinn.sad.z.maintenance.sadimport.url.store.TvinnSadMaintenanceImportUrlDataStore;
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.MaintSadImportSad006rValidator;


/**
 *  TVINN Maintenance Import Sad006r Controller 
 * 
...
@Scope("session")
public class MaintSadImportSad006rController {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSad006rController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad006r.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad006r");
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad006r_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportSadhHeadfRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad006r");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//Move on
			MaintSadImportSad006rValidator validator = new MaintSadImportSad006rValidator();
			...
			//FETCH table
			//------------
	    	List<JsonMaintSadImportSadhHeadfRecord> list = this.fetchList(appUser.getUser(), recordToValidate.getSiavd(), recordToValidate.getSitdn());
	    	//set domain objets
	    	model.put("dbTable", dbTable);
	    	model.put("avd", recordToValidate.getSiavd());
	    	model.put("opd", recordToValidate.getSitdn());
			model.put(TvinnSadMaintenanceConstants.DOMAIN_LIST, list);
			successView.addObject(TvinnSadMaintenanceConstants.DOMAIN_MODEL , model);
	 ...
	 * @param recordToValidate
	 */
	private void adjustSomeRecordValues(JsonMaintSadImportSadhHeadfRecord recordToValidate){
		final String ZERO = "0";
		//N/A
	}
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportSadhHeadfRecord> fetchList(String applicationUser, String avd, String opd){
		...
		if(avd!=null && !"".equals(avd)){
			urlRequestParams.append("&siavd=" + avd);
			if(opd!=null && !"".equals(opd)){
				urlRequestParams.append("&sitdn=" + opd);
			}
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportSadhHeadfRecord record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStore.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD006R_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportSadhHeadfContainer container = this.maintSadImportSadhHeadfService.doUpdate(jsonPayload);
	        if(container!=null){
