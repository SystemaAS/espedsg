...
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002Kodts3rValidator;
...
@Scope("session")
public class MaintSadImportSad002Kodts3Controller {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSad002Kodts3Controller.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts3r.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts3r");
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodts3Record> list = this.fetchList(appUser.getUser(), id);
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts3r_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodts3Record recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts3r");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002Kodts3rValidator validator = new MaintSadImportSad002Kodts3rValidator();
			...
			if(bindingResult.hasErrors()){
				...
				if (TvinnSadMaintenanceConstants.ACTION_UPDATE.equals(action) ){
					...
					recordToValidate.setKs3trt(null);
				}
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodts3Record> fetchList(String applicationUser, String ks3trt){
		...
		if(ks3trt!=null && !"".equals(ks3trt)){
			urlRequestParams.append("&ks3trt=" + ks3trt);
		}
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodts3Record record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTS3R_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodts3Container container = this.maintSadImportKodts3Service.doUpdate(jsonPayload);
	        if(container!=null){
