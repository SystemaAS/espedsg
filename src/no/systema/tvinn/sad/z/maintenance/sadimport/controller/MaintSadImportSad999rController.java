...
public class MaintSadImportSad999rController {
	...
	private static final Logger logger = Logger.getLogger(MaintSadImportSad999rController.class.getName());
	...
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportSadsdRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			//FETCH table
			//------------
	    	List<JsonMaintSadImportSadsdRecord> list = this.fetchList(appUser.getUser(), recordToValidate.getSdtnrf(), recordToValidate.getTaalfa(), recordToValidate.getSdkdae());
	    	//set domain objets
	    	model.put("dbTable", dbTable);
			model.put(TvinnSadMaintenanceConstants.DOMAIN_LIST, list);
			successView.addObject(TvinnSadMaintenanceConstants.DOMAIN_MODEL , model);
