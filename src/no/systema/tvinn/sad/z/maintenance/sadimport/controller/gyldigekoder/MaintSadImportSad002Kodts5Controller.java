...
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002Kodts5rValidator;
...
@Scope("session")
public class MaintSadImportSad002Kodts5Controller {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSad002Kodts5Controller.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts5r.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts5r");
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodts5Record> list = this.fetchList(appUser.getUser(), id);
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodts5r_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodts5Record recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodts5r");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002Kodts5rValidator validator = new MaintSadImportSad002Kodts5rValidator();
			...
			if(bindingResult.hasErrors()){
				...
				if (TvinnSadMaintenanceConstants.ACTION_UPDATE.equals(action) ){
					...
					recordToValidate.setKs5tln(null);
				}
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodts5Record> fetchList(String applicationUser, String ks5tln){
		...
		if(ks5tln!=null && !"".equals(ks5tln)){
			urlRequestParams.append("&ks5tln=" + ks5tln);
		}
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodts5Record record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTS5R_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodts5Container container = this.maintSadImportKodts5Service.doUpdate(jsonPayload);
	        if(container!=null){
