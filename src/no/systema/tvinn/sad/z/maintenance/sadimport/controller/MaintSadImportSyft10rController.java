...
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.MaintSadImportSyft10rValidator;
...
@Scope("session")
public class MaintSadImportSyft10rController {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSyft10rController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_syft10r.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_syft10r");
		...
		if(appUser==null){
			...
			//get table
	    	List<JsonMaintSadImportKodtsiRecord> list = this.fetchList(appUser.getUser());
	    	//set domain objets
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_syft10r_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodtsiRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_syft10r");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			MaintSadImportSyft10rValidator validator = new MaintSadImportSyft10rValidator();
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodtsiRecord> fetchList(String applicationUser){
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodtsiRecord record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStore.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SYFT10R_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodtsiContainer container = this.maintSadImportKodtsiService.doUpdate(jsonPayload);
	        if(container!=null){
