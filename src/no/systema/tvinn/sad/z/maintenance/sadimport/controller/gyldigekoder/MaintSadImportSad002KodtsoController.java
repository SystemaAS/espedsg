...
import no.systema.tvinn.sad.z.maintenance.sadimport.service.gyldigekoder.MaintSadImportKodtsoService;
import no.systema.tvinn.sad.z.maintenance.sadimport.url.store.TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder;
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002KodtsorValidator;
...
@Scope("session")
public class MaintSadImportSad002KodtsoController {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(MaintSadImportSad002KodtsoController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodtsor.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodtsor");
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodtsoRecord> list = this.fetchList(appUser.getUser(), id);
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadmaintenanceimport_sad002_kodtsor_edit.do", method={RequestMethod.GET, RequestMethod.POST})
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodtsoRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodtsor");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002KodtsorValidator validator = new MaintSadImportSad002KodtsorValidator();
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodtsoRecord> fetchList(String applicationUser, String ksefyl){
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodtsoRecord record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTSOR_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodtsoContainer container = this.maintSadImportKodtsoService.doUpdate(jsonPayload);
	        if(container!=null){
