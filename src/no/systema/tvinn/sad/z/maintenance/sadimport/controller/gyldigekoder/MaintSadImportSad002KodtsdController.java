...
import no.systema.tvinn.sad.z.maintenance.sadimport.service.gyldigekoder.MaintSadImportKodtsdService;
import no.systema.tvinn.sad.z.maintenance.sadimport.url.store.TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder;
import no.systema.tvinn.sad.z.maintenance.sadimport.validator.gyldigekoder.MaintSadImportSad002KodtsdrValidator;
...
public class MaintSadImportSad002KodtsdController {
	...
	public ModelAndView doSadMaintImportList(HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			//get table
			String id = null;
			List<JsonMaintSadImportKodtsdRecord> list = this.fetchList(appUser.getUser(), id);
	...
	public ModelAndView doSadMaintImportEdit(@ModelAttribute ("record") JsonMaintSadImportKodtsdRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tvinnsadmaintenanceimport_sad002_kodtsdr");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		...
		if(appUser==null){
			...
			//process
			MaintSadImportSad002KodtsdrValidator validator = new MaintSadImportSad002KodtsdrValidator();
	 ...
	 * @return
	 */
	private List<JsonMaintSadImportKodtsdRecord> fetchList(String applicationUser, String ksdls){
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTSDR_GET_LIST_URL;
    	...
    	//extract
    	List<JsonMaintSadImportKodtsdRecord> list = new ArrayList();
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodtsdContainer container = this.maintSadImportKodtsdService.getList(jsonPayload);
	        if(container!=null){
	 ...
	 * @return
	 */
	private int updateRecord(String applicationUser, JsonMaintSadImportKodtsdRecord record, String mode, StringBuffer errMsg){
		int retval = 0;
		
		String BASE_URL = TvinnSadMaintenanceImportUrlDataStoreGyldigeKoder.TVINN_SAD_MAINTENANCE_IMPORT_BASE_SAD002_KODTSDR_DML_UPDATE_URL;
    	...
    	if(jsonPayload!=null){
			//lists
    		JsonMaintSadImportKodtsdContainer container = this.maintSadImportKodtsdService.doUpdate(jsonPayload);
	        if(container!=null){
	...
	private MaintSadImportKodtsdService maintSadImportKodtsdService;
	@Autowired
	@Required
	public void setMaintSadImportKodtsdService (MaintSadImportKodtsdService value){ this.maintSadImportKodtsdService = value; }
	public MaintSadImportKodtsdService getMaintSadImportKodtsdService(){ return this.maintSadImportKodtsdService; }
