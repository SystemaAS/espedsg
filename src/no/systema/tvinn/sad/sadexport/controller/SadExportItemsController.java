...
public class SadExportItemsController {
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadexport_edit_items.do")
	public ModelAndView sadExportEditItem(@ModelAttribute ("record") JsonSadExportSpecificTopicItemRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		boolean bindingErrorsExist = false;
		logger.info("Inside: sadExportEditItem");
		ModelAndView successView = new ModelAndView("tvinnsadexport_edit_items");
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadexport_edit_items_autocontrol.do")
	public ModelAndView sadExportEditItemAutoControl(@ModelAttribute ("record") JsonSadExportSpecificTopicItemRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		logger.info("Inside: sadExportEditItemAutoControl");
		...
		if(appUser==null){
			...
			successView = new ModelAndView("redirect:tvinnsadexport_edit_items.do?" + params);
			//FETCH the ITEM LIST of existent ITEMs for this TOPIC
			//---------------------------
			//get BASE URL = RPG-PROGRAM
            //---------------------------
	    	...
	    	if(container!=null){
	    		...
	    		SadExportItemsAutoControlMgr autoControlMgr = new SadExportItemsAutoControlMgr(this.urlCgiProxyService, this.sadExportSpecificTopicItemService);
	    		
	    		//Iterate on every line to control the requirements
	    		for(JsonSadExportSpecificTopicItemRecord record: container.getOrderList()){
	    			//Fill in Taric Values in the record to prepare for som validity checks
	...
	public ModelAndView sadExportEditItemUpdateKundensVaruregister(@ModelAttribute ("record") KundensVareRegisterUpdateItemRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			StringBuffer params = new StringBuffer();
			params.append("action=doFetch" + "&avd=" + headerRecord.getSeavd() + "&sign=" + headerRecord.getSesg() + "&opd=" + headerRecord.getSetdn());
			successView = new ModelAndView("redirect:tvinnsadexport_edit_items.do?" + params);
	    	...
	    	if(rpgReturnResponseHandler.getErrorMessage()!=null && !"".equals(rpgReturnResponseHandler.getErrorMessage())){
	    		rpgReturnResponseHandler.setErrorMessage("[ERROR] FATAL on UPDATE: " + rpgReturnResponseHandler.getErrorMessage());
	    		this.setFatalError(model, rpgReturnResponseHandler, new JsonSadExportSpecificTopicItemRecord());
	    	}else{
	    		//Delete succefully done!
