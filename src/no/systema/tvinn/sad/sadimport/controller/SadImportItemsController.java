...
@Scope("session")
public class SadImportItemsController {
	private static final JsonDebugger jsonDebugger = new JsonDebugger();
	private static final Logger logger = Logger.getLogger(SadImportItemsController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadimport_edit_items.do")
	public ModelAndView sadImportEditItem(@ModelAttribute ("record") JsonSadImportSpecificTopicItemRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		boolean bindingErrorsExist = false;
		logger.info("Inside: sadImportEditItem");
		
		ModelAndView successView = new ModelAndView("tvinnsadimport_edit_items");
	 ...
	 * @return
	 */
	@RequestMapping(value="tvinnsadimport_edit_items_autocontrol.do")
	public ModelAndView sadImportEditItemAutoControl(@ModelAttribute ("record") JsonSadImportSpecificTopicItemRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		logger.info("Inside: sadImportEditItemAutoControl");
		...
		if(appUser==null){
			...
			params.append("user=" + appUser.getUser() + "&avd=" + recordToValidate.getSvavd() + "&opd=" + recordToValidate.getSvtdn());
			successView = new ModelAndView("redirect:tvinnsadimport_edit_items.do?" + params);
			//FETCH the ITEM LIST of existent ITEMs for this TOPIC
			//---------------------------
			//get BASE URL = RPG-PROGRAM
            //---------------------------
			String BASE_URL_FETCH = SadImportUrlDataStore.SAD_IMPORT_BASE_FETCH_TOPIC_ITEMLIST_URL;
			urlRequestParamsKeys = this.getRequestUrlKeyParameters(request, recordToValidate.getSvavd(), recordToValidate.getSvtdn(), appUser);
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
			...
			String jsonPayloadFetch = this.urlCgiProxyService.getJsonContent(BASE_URL_FETCH, urlRequestParamsKeys);
			//for debug purposes in GUI
			session.setAttribute(TvinnSadConstants.ACTIVE_URL_RPG_TVINN_SAD, BASE_URL_FETCH + "==>params: " + urlRequestParamsKeys.toString() + " " + "(fetched item list):" + jsonPayloadFetch);   
			...
			logger.info(Calendar.getInstance().getTime() +  " CGI-end timestamp");
			//logger.info("AA");
	    	JsonSadImportSpecificTopicItemContainer container = this.sadImportSpecificTopicItemService.getSadImportSpecificTopicItemContainer(jsonPayloadFetch);
	    	
	    	if(container!=null){
	    		//INIT Autocontrol Manager...
	    		SadImportItemsAutoControlMgr autoControlMgr = new SadImportItemsAutoControlMgr(this.urlCgiProxyService, this.sadImportSpecificTopicItemService);
	    		...
	    		for(JsonSadImportSpecificTopicItemRecord record: container.getOrderList()){
	    			...
	    			//this.backEndValidationOnTolltariff(appUser, record);
	    			...
	    			if(record!=null && record.isValidTolltariff()){
	    				...
	    				autoControlMgr.calculatePreferenceCode(headerRecord, appUser);
		    			autoControlMgr.calculatePvaCode(appUser);
		    			//-----------------------------------
						...
						if((avgList!=null && avgList.size()>0)){
							...
							if(this.avgiftsberakningenMgr.isMultipleChoiceAvgiftMatrix(avgList)){
								if(!this.avgiftsberakningenMgr.avgiftValuesExist(record)){
									//logger.info("WARNING.. Avg. missing ????");
									record.setMultipleChoiceAvgifter(true);
								}
							}else{
								//this is set in order to automatically set all existent single avgifter in the up-coming update without end-user intervention
								record.setSingleChoiceAvgifter(true);
								this.avgiftsberakningenMgr.calculateSingleAvgifter(appUser.getUser(), avgList, headerRecord, record);
							}
						}
						...
						if(autoControlMgr.isValidRecord()){
	    					...
	    					autoControlMgr.checkValidPreferences();
	    					if(autoControlMgr.isValidRecord()){
	    						//Go to level 3
	    						...
	    						if(autoControlMgr.isValidRecord()){
	    							...
	    							//if(autoControlMgr.isValidRecord()){
	    								//Go to level 5 - OBSOLETE since Avgifter are Optional in TVINN (according to Dachser Norway-Karin)
			    						//logger.info("level check (5) " + idDebug);
	    								//autoControlMgr.checkAvgifterMustExist();
		    							//if(autoControlMgr.isValidRecord()){
		    							...
		    							/*}else{
		    					 			//Set error
		    								logger.info(ERROR_FRAME_STD_OUTPUT);
		    	    						logger.info("ERROR level (5) - Avgifger multiple choice is mandatory" + idDebug);
		    	    						logger.info(ERROR_FRAME_STD_OUTPUT);
		    	    						autoControlMgr.updateItemWithAutoControlError(appUser.getUser(), AUTO_CONTROL_ERROR_FLAG_VALUE);
		    							}*/
	    							...
	    							/*}else{
	    								//Set error
	    								logger.info(ERROR_FRAME_STD_OUTPUT);
	    	    						logger.info("ERROR level (4) - Mengde Must Not Exist" + idDebug);
	    	    						logger.info(ERROR_FRAME_STD_OUTPUT);
	    	    						autoControlMgr.updateItemWithAutoControlError(appUser.getUser(), AUTO_CONTROL_ERROR_FLAG_VALUE);
	    							}*/
	...
	public ModelAndView sadImportEditItemUpdateKundensVaruregister(@ModelAttribute ("record") KundensVareRegisterUpdateItemRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			StringBuffer params = new StringBuffer();
			params.append("action=doFetch" + "&avd=" + headerRecord.getSiavd() + "&sign=" + headerRecord.getSisg() + "&opd=" + headerRecord.getSitdn());
			successView = new ModelAndView("redirect:tvinnsadimport_edit_items.do?" + params);
	    	...
	    	if(rpgReturnResponseHandler.getErrorMessage()!=null && !"".equals(rpgReturnResponseHandler.getErrorMessage())){
	    		rpgReturnResponseHandler.setErrorMessage("[ERROR] FATAL on UPDATE: " + rpgReturnResponseHandler.getErrorMessage());
	    		this.setFatalError(model, rpgReturnResponseHandler, new JsonSadImportSpecificTopicItemRecord());
	    	}else{
	    		//Delete succefully done!
