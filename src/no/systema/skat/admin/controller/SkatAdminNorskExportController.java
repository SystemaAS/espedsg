...
import no.systema.skat.admin.validator.SkatAdminNorskExportListValidator;
...
/**
 * SKAT Admin Norsk Export Controller 
 * 
...
public class SkatAdminNorskExportController {
	 ...
	 * @return
	 */
	@RequestMapping(value="skatadmin_norskexport.do", method=RequestMethod.GET)
	public ModelAndView doInit(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("skatadmin_norskexport");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		SearchFilterSkatAdminNorskExportList searchFilter = new SearchFilterSkatAdminNorskExportList();
	 ...
	 * @return
	 */
	@RequestMapping(value="skatadmin_norskexport", params="action=doFind",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doFind(@ModelAttribute ("record") SearchFilterSkatAdminNorskExportList recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			//Validation
			//-----------
			SkatAdminNorskExportListValidator validator = new SkatAdminNorskExportListValidator();
			...
			if(bindingResult.hasErrors()){
	            ...
	            //get BASE URL
		    		final String BASE_URL = SkatAdminUrlDataStore.SKAT_ADMIN_BASE_NORSKEXPORT_LIST_URL;
		    		//add URL-parameters
				...
				logger.info(jsonPayload);
			    	...
			    	if(jsonPayload!=null){
			    		JsonSkatAdminNorskExportListContainer jsonSkatAdminNorskExportListContainer = this.skatAdminTransportService.getSkatAdminNorskExportListContainer(jsonPayload);
					//-----------------------------------------------------------
					//now filter the topic list with the search filter (if applicable)
					//-----------------------------------------------------------
					outputList = jsonSkatAdminNorskExportListContainer.getSadeksportlista();
	 ...
	 * @return
	 */
	private String getRequestUrlKeyParameters(SearchFilterSkatAdminNorskExportList searchFilter, SystemaWebUser appUser){
		StringBuffer urlRequestParamsKeys = new StringBuffer();
