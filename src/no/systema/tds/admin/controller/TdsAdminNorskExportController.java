...
import no.systema.tds.admin.validator.TdsAdminNorskExportListValidator;
...
/**
 * TDS Admin Norsk Export Controller 
 * 
...
@Scope("session")
public class TdsAdminNorskExportController {
	
	private static final Logger logger = Logger.getLogger(TdsAdminNorskExportController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tdsadmin_norskexport.do", method=RequestMethod.GET)
	public ModelAndView doInit(HttpSession session, HttpServletRequest request){
		ModelAndView successView = new ModelAndView("tdsadmin_norskexport");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		SearchFilterTdsAdminNorskExportList searchFilter = new SearchFilterTdsAdminNorskExportList();
	 ...
	 * @return
	 */
	@RequestMapping(value="tdsadmin_norskexport", params="action=doFind",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doFind(@ModelAttribute ("record") SearchFilterTdsAdminNorskExportList recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			//Validation
			//-----------
			TdsAdminNorskExportListValidator validator = new TdsAdminNorskExportListValidator();
			...
			if(bindingResult.hasErrors()){
	            ...
	            //get BASE URL
		    		final String BASE_URL = TdsAdminUrlDataStore.TDS_ADMIN_BASE_NORSKEXPORT_LIST_URL;
		    		//add URL-parameters
				...
				session.setAttribute(TdsConstants.ACTIVE_URL_RPG, BASE_URL + "==>params: " + urlRequestParams.toString()); 
			    	...
			    	//tmp fix
			    	jsonPayload = jsonPayload.replaceAll("sadeksport lista", "sadeksportlista");
				
				//Debug --> 
				logger.info(jsonPayload);
			    	logger.info(Calendar.getInstance().getTime() +  " CGI-end timestamp");
			    	if(jsonPayload!=null){
			    		JsonTdsAdminNorskExportListContainer jsonTdsAdminNorskExportListContainer = this.tdsAdminTransportService.getTdsAdminNorskExportListContainer(jsonPayload);
					//-----------------------------------------------------------
					//now filter the topic list with the search filter (if applicable)
					//-----------------------------------------------------------
					outputList = jsonTdsAdminNorskExportListContainer.getSadeksportlista();
	...
	private void populateAvdelningHtmlDropDownsFromJsonString(Map model, SystemaWebUser appUser){
		...
		String UPPDRAG_TYPE = "A"; //All avd in the company (for SAD-export)
		try{
	...
	private void populateSignatureHtmlDropDownsFromJsonString(Map model, SystemaWebUser appUser){
		//fill in html lists here
		String UPPDRAG_TYPE = "F"; //TDS (TODO for SAD-import ?) 
	 ...
	 * @return
	 */
	private String getRequestUrlKeyParameters(SearchFilterTdsAdminNorskExportList searchFilter, SystemaWebUser appUser){
		StringBuffer urlRequestParamsKeys = new StringBuffer();
