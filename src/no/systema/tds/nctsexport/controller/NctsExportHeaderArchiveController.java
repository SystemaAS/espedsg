...
/**
 * 
 * NCTS Export Topic Archive Controller 
 * 
...
@Controller
@Scope("session")
public class NctsExportHeaderArchiveController {
	
	private static final Logger logger = Logger.getLogger(NctsExportHeaderArchiveController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="nctsexport_archive.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("nctsexport_archive");
		logger.info("Method: doShowList [RequestMapping-->nctsexport_archive.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = UrlDataStore.NCTS_EXPORT_BASE_ARCHIVE_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonNctsExportSpecificTopicArchiveContainer jsonNctsExportSpecificTopicArchiveContainer = this.nctsExportSpecificTopicService.getNctsExportSpecificTopicArchiveContainer(jsonPayload);
		    		//add domain objects here
		    		this.setDomainObjectsInView(model, jsonNctsExportSpecificTopicArchiveContainer);
		    		this.setDomainObjectsInView(request, model);
		    		
		    		successView.addObject(TdsConstants.DOMAIN_MODEL, model);
				successView.addObject(TdsConstants.DOMAIN_LIST,jsonNctsExportSpecificTopicArchiveContainer.getArchiveElements());
	 ...
	 * @return
	 */
	@RequestMapping(value="ncts_export_renderArchive.do", method={ RequestMethod.GET })
	public ModelAndView doNctsExportRenderArchive(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doNctsExportRenderArchive...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
	 ...
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonNctsExportSpecificTopicArchiveRecord jsonNctsExportSpecificTopicArchiveRecord){
		logger.info(rpgReturnResponseHandler.getErrorMessage());
		this.setAspectsInView(model, rpgReturnResponseHandler);
		//No refresh on jsonRecord is done for the GUI (form fields). Must be implemented right here, if required. !!
        this.setDomainObjectsInView(model, jsonNctsExportSpecificTopicArchiveRecord);
	}
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonNctsExportSpecificTopicArchiveContainer jsonNctsExportSpecificTopicArchiveContainer){
		//SET HEADER RECORDS  (from RPG)
		for (JsonNctsExportSpecificTopicArchiveRecord record : jsonNctsExportSpecificTopicArchiveContainer.getArchiveElements()){
	 ...
	 * @param jsonNctsExportSpecificTopicRecord
	 */
	private void setDomainObjectsInView(Map model, JsonNctsExportSpecificTopicArchiveRecord jsonNctsExportSpecificTopicArchiveRecord){
		//SET HEADER RECORDS  (from RPG)
		model.put(TdsConstants.DOMAIN_RECORD, jsonNctsExportSpecificTopicArchiveRecord);
	}
	/**
	 * Sets aspects 
