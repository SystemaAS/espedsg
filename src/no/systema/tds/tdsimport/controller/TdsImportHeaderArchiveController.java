...
/**
 * 
 * TDS Import Topic Archive Controller 
 * 
...
@Controller
@Scope("session")
public class TdsImportHeaderArchiveController {
	
	private static final Logger logger = Logger.getLogger(TdsImportHeaderArchiveController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tdsimport_archive.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("tdsimport_archive");
		logger.info("Method: doShowList [RequestMapping-->tdsimport_archive.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = TdsImportUrlDataStore.TDS_IMPORT_BASE_ARCHIVE_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonTdsImportSpecificTopicArchiveContainer jsonTdsImportSpecificTopicArchiveContainer = this.tdsImportSpecificTopicService.getTdsImportSpecificTopicArchiveContainer(jsonPayload);
		    		//add domain objects here
		    		this.setDomainObjectsInView(model, jsonTdsImportSpecificTopicArchiveContainer);
		    		this.setDomainObjectsInView(request, model);
		    		
		    		successView.addObject(TdsConstants.DOMAIN_MODEL, model);
				successView.addObject(TdsConstants.DOMAIN_LIST,jsonTdsImportSpecificTopicArchiveContainer.getArchiveElements());
	 ...
	 * @param record
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonTdsImportSpecificTopicArchiveRecord record){
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonTdsImportSpecificTopicArchiveContainer container){
		//SET HEADER RECORDS  (from RPG)
		for (JsonTdsImportSpecificTopicArchiveRecord record : container.getArchiveElements()){
	 ...
	 * @param record
	 */
	private void setDomainObjectsInView(Map model, JsonTdsImportSpecificTopicArchiveRecord record){
		//SET HEADER RECORDS  (from RPG)
		model.put(TdsConstants.DOMAIN_RECORD, record);
	}
	/**
	...
	@Autowired
	@Required
	public void setTdsExportSpecificTopicService (TdsImportSpecificTopicService value){ this.tdsImportSpecificTopicService = value; }
	public TdsImportSpecificTopicService getTdsExportSpecificTopicService(){ return this.tdsImportSpecificTopicService; }
