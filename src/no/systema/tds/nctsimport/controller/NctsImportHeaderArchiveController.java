...
/**
 * 
 * NCTS Import Topic Archive Controller 
 * 
...
@Controller
@Scope("session")
public class NctsImportHeaderArchiveController {
	
	private static final Logger logger = Logger.getLogger(NctsImportHeaderArchiveController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="nctsimport_archive.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("nctsimport_archive");
		logger.info("Method: doShowList [RequestMapping-->nctsimport_archive.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = UrlDataStore.NCTS_IMPORT_BASE_ARCHIVE_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonNctsImportSpecificTopicArchiveContainer jsonNctsImportSpecificTopicArchiveContainer = this.nctsImportSpecificTopicService.getNctsImportSpecificTopicArchiveContainer(jsonPayload);
		    		//add domain objects here
		    		this.setDomainObjectsInView(model, jsonNctsImportSpecificTopicArchiveContainer);
		    		this.setDomainObjectsInView(request, model);
		    		
		    		successView.addObject(TdsConstants.DOMAIN_MODEL, model);
				successView.addObject(TdsConstants.DOMAIN_LIST,jsonNctsImportSpecificTopicArchiveContainer.getArchiveElements());
	 ...
	 * @return
	 */
	@RequestMapping(value="ncts_import_renderArchive.do", method={ RequestMethod.GET })
	 ...
	 * @param record
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonNctsImportSpecificTopicArchiveRecord record){
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonNctsImportSpecificTopicArchiveContainer container){
		//SET HEADER RECORDS  (from RPG)
		for (JsonNctsImportSpecificTopicArchiveRecord record : container.getArchiveElements()){
	 ...
	 * @param record
	 */
	private void setDomainObjectsInView(Map model, JsonNctsImportSpecificTopicArchiveRecord record){
		//SET HEADER RECORDS  (from RPG)
		model.put(TdsConstants.DOMAIN_RECORD, record);
	}
	/**
