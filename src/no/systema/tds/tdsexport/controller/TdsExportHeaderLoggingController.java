...
@Controller
@Scope("session")
public class TdsExportHeaderLoggingController {
	
	private static final Logger logger = Logger.getLogger(TdsExportHeaderLoggingController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tdsexport_logging.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("tdsexport_logging");
		logger.info("Method: doShowList [RequestMapping-->tdsexport_logging.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = TdsExportUrlDataStore.TDS_EXPORT_BASE_LOG_LIST_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonTdsExportSpecificTopicLoggingContainer jsonTdsExportSpecificTopicLoggingContainer = this.tdsExportSpecificTopicService.getTdsExportSpecificTopicLoggingContainer(jsonPayload);
		    		//add domain objects here
		    		this.setDomainObjectsInView(model, jsonTdsExportSpecificTopicLoggingContainer);
		    		this.setDomainObjectsInView(request, model);
		    		
		    		successView.addObject(TdsConstants.DOMAIN_MODEL, model);
				successView.addObject(TdsConstants.DOMAIN_LIST,jsonTdsExportSpecificTopicLoggingContainer.getLogg());
	 ...
	 * @return
	 */
	@RequestMapping(value="tds_export_renderEdifact.do", method={ RequestMethod.GET })
	public ModelAndView doTdsExportRenderEdifact(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		logger.info("Inside doTdsExportRenderEdifact...");
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
	 ...
	 * @return
	 */
	@RequestMapping(value="tds_export_renderLargeText.do",  method= RequestMethod.GET)
	public ModelAndView doTdsExportRenderLargeText(HttpSession session, HttpServletRequest request, HttpServletResponse response){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		logger.info("Inside doTdsExportRenderLargeText...");
		...
		if(appUser==null){
			...
			//get BASE URL = RPG-PROGRAM
            //---------------------------
			String BASE_URL = TdsExportUrlDataStore.TDS_EXPORT_BASE_LOG_LARGE_TEXT_FOR_SPECIFIC_TOPIC_URL;
			//url params
	...
	private String setLargeTextInDomainObject(String jsonPayload){
		...
		JsonTdsExportSpecificTopicLoggingLargeTextContainer container = this.tdsExportSpecificTopicService.getTdsExportSpecificTopicLoggingLargeTextContainer(jsonPayload);
	 ...
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonTdsExportSpecificTopicLoggingRecord jsonTdsExportSpecificTopicLoggingRecord){
		logger.info(rpgReturnResponseHandler.getErrorMessage());
		this.setAspectsInView(model, rpgReturnResponseHandler);
		//No refresh on jsonRecord is done for the GUI (form fields). Must be implemented right here, if required. !!
        this.setDomainObjectsInView(model, jsonTdsExportSpecificTopicLoggingRecord);
	}
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonTdsExportSpecificTopicLoggingContainer jsonTdsExportSpecificTopicLoggingContainer){
		//SET HEADER RECORDS  (from RPG)
		for (JsonTdsExportSpecificTopicLoggingRecord record : jsonTdsExportSpecificTopicLoggingContainer.getLogg()){
	 ...
	 * @param jsonTdsExportSpecificTopicRecord
	 */
	private void setDomainObjectsInView(Map model, JsonTdsExportSpecificTopicLoggingRecord jsonTdsExportSpecificTopicLoggingRecord){
		//SET HEADER RECORDS  (from RPG)
		model.put(TdsConstants.DOMAIN_RECORD, jsonTdsExportSpecificTopicLoggingRecord);
	}
	/**
	 * Sets aspects 
