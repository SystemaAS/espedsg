...
/**
 * 
 * TDS Export Topic Archive Controller 
 * 
...
@Controller
@Scope("session")
public class TdsExportHeaderArchiveController {
	
	private static final Logger logger = Logger.getLogger(TdsExportHeaderArchiveController.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tdsexport_archive.do",  method= RequestMethod.GET)
	public ModelAndView doShowList(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		//String messageFromContext = this.context.getMessage("user.label",new Object[0], request.getLocale());
		ModelAndView successView = new ModelAndView("tdsexport_archive");
		logger.info("Method: doShowList [RequestMapping-->tdsexport_archive.do]");
		...
		if(appUser==null){
            ...
            //---------------------------
			String BASE_URL = TdsExportUrlDataStore.TDS_EXPORT_BASE_ARCHIVE_FOR_SPECIFIC_TOPIC_URL;
			//url params
			...
			logger.info(Calendar.getInstance().getTime() + " CGI-start timestamp");
		    	...
		    	if(jsonPayload!=null){
		    		JsonTdsExportSpecificTopicArchiveContainer jsonTdsExportSpecificTopicArchiveContainer = this.tdsExportSpecificTopicService.getTdsExportSpecificTopicArchiveContainer(jsonPayload);
		    		//add domain objects here
		    		this.setDomainObjectsInView(model, jsonTdsExportSpecificTopicArchiveContainer);
		    		this.setDomainObjectsInView(request, model);
		    		
		    		successView.addObject(TdsConstants.DOMAIN_MODEL, model);
				successView.addObject(TdsConstants.DOMAIN_LIST,jsonTdsExportSpecificTopicArchiveContainer.getArchiveElements());
	 ...
	 */
	private void setFatalError(Map model, RpgReturnResponseHandler rpgReturnResponseHandler, JsonTdsExportSpecificTopicArchiveRecord jsonTdsExportSpecificTopicArchiveRecord){
		logger.info(rpgReturnResponseHandler.getErrorMessage());
		this.setAspectsInView(model, rpgReturnResponseHandler);
		//No refresh on jsonRecord is done for the GUI (form fields). Must be implemented right here, if required. !!
        this.setDomainObjectsInView(model, jsonTdsExportSpecificTopicArchiveRecord);
	}
	 ...
	 * @return
	 */
	private void setDomainObjectsInView(Map model, JsonTdsExportSpecificTopicArchiveContainer jsonTdsExportSpecificTopicArchiveContainer){
		//SET HEADER RECORDS  (from RPG)
		for (JsonTdsExportSpecificTopicArchiveRecord record : jsonTdsExportSpecificTopicArchiveContainer.getArchiveElements()){
	 ...
	 * @param jsonTdsExportSpecificTopicRecord
	 */
	private void setDomainObjectsInView(Map model, JsonTdsExportSpecificTopicArchiveRecord jsonTdsExportSpecificTopicArchiveRecord){
		//SET HEADER RECORDS  (from RPG)
		model.put(TdsConstants.DOMAIN_RECORD, jsonTdsExportSpecificTopicArchiveRecord);
	}
	/**
	 * Sets aspects 
