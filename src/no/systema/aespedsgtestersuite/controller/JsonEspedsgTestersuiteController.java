...
public class JsonEspedsgTestersuiteController {
	...
	public ModelAndView doTestGeneric( HttpSession session, HttpServletRequest request, HttpServletResponse response){
		...
		if(appUser==null){
			...
			list = this.setTesterSuiteServices(testModule, model);
	 ...
	 * @return
	 */
	private List setTesterSuiteServices(String testModule, Map<String, Object> model){
					...
					if(value!=null && !"".equals(value)){
						...
						if(fireFirstSmokeTest(value, testersuiteObject, this.TIMEOUT_LIMIT)){
							if(value.contains("syj") && field.getName().contains("UPDATE") ){
								//java services with Update require a valid USER and get a NullPointerException otherwise. 
								...
								String jsonReturnPayload = this.urlCgiProxyService.getJsonContent(BASE_URL, "");
