...
/**
 * Tror - Order Header FlygImport Controller 
 * 
...
public class TrorMainOrderHeaderFlyImportController {
	...
	private final String DELSYSTEM_FLY_IMPORT = "C";
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport.do",  params="action=doInit", method={RequestMethod.GET, RequestMethod.POST })
	public ModelAndView doInit(@ModelAttribute ("record") JsonTrorOrderHeaderRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("tror_mainorderflyimport");
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport.do", method={RequestMethod.GET, RequestMethod.POST} )
		...
		if(appUser==null){
			...
			if(strMgr.isNotNull(recordToValidate.getHeopd()) && strMgr.isNotNull(recordToValidate.getHeavd()) ){
				if(isValidRecord){
					...
					session.setAttribute(TrorConstants.SESSION_SUBSYSTEM_ORDER_TROR, TrorConstants.SESSION_SUBSYSTEM_ORDER_TROR_FLYIMPORT);
				}else{
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport_copy.do", method={RequestMethod.GET, RequestMethod.POST} )
	...
	private void adjustDefaultValues(JsonTrorOrderHeaderRecord recordToValidate, SystemaWebUser appUser ){
		...
		recordToValidate.setHeur(this.DELSYSTEM_FLY_IMPORT);
		//split godsnr
	...
	private JsonTrorOrderHeaderRecord getDefaultValuesFromDbDummy(JsonTrorOrderHeaderRecord recordToValidate, SystemaWebUser appUser ){
    	...
    	if(jsonPayload!=null){
    		JsonTrorOrderHeaderDummyContainer container = this.trorMainOrderHeaderFlyimportService.getOrderHeaderDummyContainer(jsonPayload);
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport_updateStatus.do", method={RequestMethod.GET, RequestMethod.POST })
	public ModelAndView doUpdateStatus(@ModelAttribute ("record") JsonTrorOrderHeaderRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(strMgr.isNotNull(opd) && strMgr.isNotNull(avd)){
			...
			successView = new ModelAndView("redirect:tror_mainorderflyimport.do?action=doFetch&heavd=" + avd + "&heopd=" + opd);
		}else{
		...
		if(appUser==null){
	    	...
	    	if(jsonPayload!=null){
	    		...
	    		JsonTrorOrderHeaderContainer container = this.trorMainOrderHeaderFlyimportService.getOrderHeaderContainerStatusUpdate(jsonPayload);
	    		if(container!=null){
	...
	private int updateRecord(Map model, String applicationUser, JsonTrorOrderHeaderRecord recordToValidate, String mode, StringBuffer errMsg){
    	...
    	if(jsonPayload!=null){
    		JsonTrorOrderHeaderContainer container = this.trorMainOrderHeaderFlyimportService.getOrderHeaderContainer(jsonPayload);
    		if(container!=null){
