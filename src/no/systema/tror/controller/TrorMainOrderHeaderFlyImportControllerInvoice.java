...
import no.systema.tror.model.jsonjackson.order.invoice.JsonTrorOrderFlyImportInvoiceReadyMarkContainer;
...
@Scope("session")
public class TrorMainOrderHeaderFlyImportControllerInvoice {
	private static final JsonDebugger jsonDebugger = new JsonDebugger(1500);
	private static Logger logger = Logger.getLogger(TrorMainOrderHeaderFlyImportControllerInvoice.class.getName());
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport_invoice.do", method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doGetInvoice(@ModelAttribute ("record") JsonTrorOrderHeaderRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("tror_mainorderflyimport_invoice");
		...
		if(appUser==null){
			...
			if(recordToValidate.getHeavd()!=null && !"".equals(recordToValidate.getHeavd()) && 
	    		...
	    		if(jsonPayload!=null){
	    			JsonTrorOrderFlyImportInvoiceContainer container = this.trorMainOrderHeaderFlyimportService.getOrderInvoiceContainer(jsonPayload);
	    			if(container!=null){
	    				//ready mark status
	    				JsonTrorOrderFlyImportInvoiceReadyMarkContainer readyMarkContainer = this.getReadyMark(appUser, recordToValidate.getHeavd(), recordToValidate.getHeopd());
	 ...
	 * @return
	 */
	private   JsonTrorOrderFlyImportInvoiceReadyMarkContainer getReadyMark(SystemaWebUser appUser, String avd, String opd ){
		JsonTrorOrderFlyImportInvoiceReadyMarkContainer retval = null;
		...
		if(jsonPayload!=null){
			...
			JsonTrorOrderFlyImportInvoiceReadyMarkContainer container = new JsonTrorOrderFlyImportInvoiceReadyMarkContainer();
	 ...
	 * @return
	 */
	@RequestMapping(value="tror_mainorderflyimport_invoice_edit.do",  method={RequestMethod.GET, RequestMethod.POST} )
	public ModelAndView doEditInvoice(@ModelAttribute ("record") JsonTransportDispWorkflowSpecificOrderInvoiceRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		ModelAndView successView = new ModelAndView("redirect:tror_mainorderflyimport_invoice.do?action=doFind&heavd=" + heavd + "&heopd=" + heopd + "&itemsType=O");
		ModelAndView errorView = new ModelAndView("tror_mainorderflyimport_invoice");
	 ...
	 * @param session
	 */
	private void setDomainObjectsInView(Map model, JsonTrorOrderFlyImportInvoiceContainer container, HttpSession session){
		List<JsonTrorOrderFlyImportInvoiceRecord> list = new ArrayList<JsonTrorOrderFlyImportInvoiceRecord>();
		...
		Collections.sort(list, new JsonTrorOrderFlyImportInvoiceRecord.OrderByCodeAndInvoiceNr());
		if(list!=null){
		...
		//DEBUG
		/*for(JsonTrorOrderFlyImportInvoiceRecord record : list){
	 ...
	 * @return
	 */
	private List<JsonTrorOrderFlyImportInvoiceRecord> getListWithTotals(Map<Integer, List<JsonTrorOrderFlyImportInvoiceRecord>> listGroupsMap){
		List<JsonTrorOrderFlyImportInvoiceRecord> listWithTotals = new ArrayList<JsonTrorOrderFlyImportInvoiceRecord>();
		int TWO_DECIMALS = 2;
	    ...
	    while (it.hasNext()) {
	        ...
	        List<JsonTrorOrderFlyImportInvoiceRecord> group = (List<JsonTrorOrderFlyImportInvoiceRecord>)pair.getValue();
	        Double fablnTotal = 0.00D;
	        for (JsonTrorOrderFlyImportInvoiceRecord groupRecord : group){
	 ...
	 * @return
	 */
	private Map<Integer,List<JsonTrorOrderFlyImportInvoiceRecord>> setListGroups(List<JsonTrorOrderFlyImportInvoiceRecord> list){
		Map<Integer,List<JsonTrorOrderFlyImportInvoiceRecord>> map = new HashMap<Integer, List<JsonTrorOrderFlyImportInvoiceRecord>>();
		List<JsonTrorOrderFlyImportInvoiceRecord> newList = new ArrayList<JsonTrorOrderFlyImportInvoiceRecord>();
		String previousCode = "";
		int index = 0;
		Integer mapIndex = 0;
		for (JsonTrorOrderFlyImportInvoiceRecord record: list){
			...
			if (index==0){
				...
				if (previousCode.equals(record.getFask() + record.getFafakt())){
					...
					mapIndex++;
					newList = new ArrayList<JsonTrorOrderFlyImportInvoiceRecord>();
					newList.add(record);
					map.put(mapIndex, newList);
				}
				previousCode = record.getFask() + record.getFafakt();
	 ...
	 * @return
	 */
	private JsonTrorOrderFlyImportInvoiceRecord addTotalRecord(){
		JsonTrorOrderFlyImportInvoiceRecord record = new JsonTrorOrderFlyImportInvoiceRecord();
